 ...
 *****************************************************************************/
package org.dusfan.compiere.impexp;
 ...
 *  @version $Id$
 */
public final class ImpFormatNumidia
{
	/**
	 *	Format
	 *  @param name name
	 ...
	 */
	public ImpFormatNumidia (String name, int AD_Table_ID, String formatType)
	{
		setName(name);
	...
	/**	Logger			*/
	private static CLogger	log = CLogger.getCLogger(ImpFormatNumidia.class);
	 ...
	 */
	public static ImpFormatNumidia load (int Id)
	{
		if (log.isLoggable(Level.CONFIG))log.config(String.valueOf(Id));
		ImpFormatNumidia retValue = null;
		...
		try
		{
			...
			if (rs.next())
			{
				retValue = new ImpFormatNumidia (rs.getString("Name"), rs.getInt("AD_Table_ID"), rs.getString("FormatType"));
	 ...
	 *  @param ID id
	 */
	private static void loadRows (ImpFormatNumidia format, int ID)
	{
	...
	//	log.config( "ImpFormat.updateDB - listSize=" + nodes.length);
		...
		// here we are going to add group
		int du_Visa_Group_ID = getANDcreateGroup(nodes, trxName, ctx, AD_Client_ID, AD_Org_ID, UpdatedBy);
		// if the group don't add return false
		...
		if (du_Visa_Group_ID > 1)
			sql.append("du_Visa_Group_ID = " + du_Visa_Group_ID + ", ");
	...
	private int getANDcreateGroup (String[] nodes, String trxName,
			Properties ctx, int ad_client_id , int ad_org_id, int UpdatedBy) {
		try {
			String groupName = "";
			String codeClient = "";

			for (int i = 0; i < nodes.length; i++) {
				String target = nodes[i];
				if (target.toUpperCase().contains("GroupName".toUpperCase()))
					groupName = target;
				if (target.toUpperCase().contains("GroupCode".toUpperCase()))
					codeClient = target;
				if (groupName.length() > 0 && codeClient.length() > 0)
					break;
			}

			groupName = groupName.substring(groupName.indexOf("=") + 1).replace("'", "");
			codeClient = codeClient.substring(codeClient.indexOf("=") + 1).replace("'", "");
			// if the record is header don't do any things
			if (groupName.equalsIgnoreCase("GroupName") || codeClient.equalsIgnoreCase("GroupCode"))
				return 1;
			
			String sql = "Select du_Visa_Group_ID from du_Visa_Group where value = '" + groupName + "'";
			int du_Visa_Group_ID = DB.getSQLValue(trxName, sql);
			// if the group exist return directly the group id
			if (du_Visa_Group_ID > 0) {
				return du_Visa_Group_ID;
			} else 
			{ // if the group not exist create it
				MVisaGroup gvisa = new MVisaGroup(ctx, 0, trxName);
				gvisa.setValue(groupName);
				gvisa.set_ValueNoCheck("AD_Client_ID", ad_client_id);
				gvisa.setAD_Org_ID(ad_org_id);
				// Set code client to visa groupe
				int c_BPartner_ID = DB.getSQLValue(trxName, "Select c_bpartner_id from c_bpartner where value = '" + codeClient + "'");
				if (c_BPartner_ID > 0)
					gvisa.set_ValueNoCheck("C_BPartnerRelation_ID", c_BPartner_ID);
				if (gvisa.save())
					return gvisa.getDU_Visa_Group_ID();
				
//				int ID = DB.getNextID(ctx, "DU_Visa_Group", null);
//				sql = " insert into du_visa_group "
//						+ " (ad_org_id, updated, isactive, ad_client_id, created, createdby,updatedby, value, c_bpartnerrelation_id, du_visa_group_id) "
//						+ " values " + " (" + ad_org_id + " , current_date, 'Y', " + ad_client_id + ", "
//						+ " current_date, " + UpdatedBy + "," + UpdatedBy + ",'" + groupName + "', "
//						+ " (Select c_bpartner_id from c_bpartner where value ='" + codeClient + "') " + " ," + ID
//						+ ") ";
//				int no = DB.executeUpdate(sql.toString(), trxName);
//				if (no > 0) {
//					sql = "Select du_Visa_Group_ID from du_Visa_Group where value ='" + groupName + "'";
//					du_Visa_Group_ID = DB.getSQLValue(trxName, sql);
//					if (du_Visa_Group_ID > 0)
//						return du_Visa_Group_ID;
//				}

...
public class DUWFileImportNumidia extends ADForm implements EventListener<Event>
{
	/**
	 * 
	 */
	...
	private ImpFormatNumidia m_format;
	...
	private void dynInit()
	{
		...
		// Set default format import to omra format -Umrah-
		for (int i = 0; i < pickFormat.getItemCount(); i++) {
			ListItem listitem = pickFormat.getItemAtIndex(i);
			String format = listitem.getLabel().toString();
			if (format.contains("Umrah")) {
		...
		Charset charset = Charset.forName("windows-1256"); // set default to read arabic
	...
	private void cmd_loadFormat()
	{
		...
		m_format = ImpFormatNumidia.load (formatId);
	...
	private void cmd_process()
	{
		...
		if (checkGroupExist(bFile.getLabel())) {
			FDialog.error(m_WindowNo, this, "Le groupe existe,Contacter l administrateur");
			return;
		}
	...
	private boolean checkGroupExist (String group) {
		
		String grp = group.substring(0, group.indexOf(".csv"));
		String sql = "Select du_Visa_Group_ID from du_Visa_Group where value = '" + grp + "'";
		int du_Visa_Group_ID = DB.getSQLValue(null, sql);

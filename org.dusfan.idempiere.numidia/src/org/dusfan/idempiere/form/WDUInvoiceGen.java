...
import org.adempiere.webui.apps.form.WInvoiceGen;
...
import org.compiere.apps.form.DUInvoiceGen;
...
public class WDUInvoiceGen extends DUInvoiceGen implements IFormController, EventListener<Event>, ValueChangeListener{
	...
	private Label lvol = new Label(); // vol
	private WTableDirEditor fvol; // vol

	public WDUInvoiceGen()
	{
		log.info("");
	...
	void zkInit() throws Exception
	{
		...
		lBPartner.setText("Code Client/" + Msg.translate(Env.getCtx(), "C_BPartner_ID"));
		...
		row.appendCellChild(new Space());
		row.appendCellChild(lvol.rightAlign());
		ZKUpdateUtil.setHflex(fvol.getComponent(), "true");
		row.appendCellChild(fvol.getComponent());
		row.appendCellChild(new Space());
	}	//	jbInit
	...
	//	lBPartner.setText(Msg.translate(Env.getCtx(), "C_BPartner_ID"));
		...
		// Select the flight to generate invoice only for that flight
		lvol.setText("VOL");
		MLookup product = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), DisplayType.TableDir ,
				1000425, Env.getLanguage(Env.getCtx()), "DU_Vol_ID", -1 ,
				false, " DU_Vol.isActive='Y'");
		fvol = new WTableDirEditor("DU_Vol_ID", false, false, true, product);
		fvol.addValueChangeListener(this);
		// end flight

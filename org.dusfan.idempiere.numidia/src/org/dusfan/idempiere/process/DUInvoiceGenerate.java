...
public class DUInvoiceGenerate extends SvrProcess {
	...
	protected String doIt () throws Exception
	{
		...
		if (p_Selection)	//	VInvoiceGen
		{
			sql = new StringBuilder("SELECT C_Order.* FROM C_Order, T_Selection ")
				...
				.append("ORDER BY C_Order.M_Warehouse_ID, C_Order.PriorityRule, C_Order.Bill_BPartner_ID, C_Order.Bill_Location_ID, C_Order.C_Order_ID");
		}
		else
		{
			...
			if (p_C_BPartner_ID != 0)
				sql.append(" AND Bill_BPartner_ID=?"); // Code Client
			if (p_C_Order_ID != 0)
				sql.append(" AND C_Order_ID=?");
			if (p_M_Product_ID != 0) {
				String sqlv = " AND exists (select 1 from "

 ...
 *	
 **/
public class BankTransferFixced extends SvrProcess
{
	...
	private void generateBankTransfer()
	{
		...
		paymentBankFrom.setTenderType((String) mBankFrom.get_Value("TenderType"));
		...
		paymentBankFrom.setC_DocType_ID(getC_Doctype_IDbyAccount(mBankFrom, false));
		...
		paymentBankTo.setTenderType((String) mBankTo.get_Value("TenderType"));
		...
		paymentBankTo.setC_DocType_ID(getC_Doctype_IDbyAccount(mBankTo, true));
	...
	private int getC_Doctype_IDbyAccount (MBankAccount mbank, boolean isReceipt) {
		String sql = "SELECT C_DocType_ID FROM C_DocType WHERE IsActive='Y' "
				+ "AND AD_Client_ID=? AND DocBaseType=? AND TenderType=? ORDER BY IsDefault DESC";
		...
		try
		{
			...
			pstmt.setString(3, (String) mbank.get_Value("TenderType"));

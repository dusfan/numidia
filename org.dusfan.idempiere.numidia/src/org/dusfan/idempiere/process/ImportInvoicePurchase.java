...
import org.dusfan.idempiere.model.X_I_InvoicePurchase;
 ...
 */
public class ImportInvoicePurchase extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		{
			sql = new StringBuilder ("DELETE I_InvoicePurchase ")
...
//		Customization LoH
		...
		sql = new StringBuilder ("delete from I_InvoicePurchase o ")
				.append("WHERE DocumentNo is NULL or PendingPayment is null or DocumentNo like 'Booking%'")
		...
		sql = new StringBuilder ("UPDATE I_InvoicePurchase o ")
				.append(" SET QtyOrdered = 1, C_PaymentTerm_ID=1000006,")
				.append(" C_Currency_ID = (SELECT C_Currency_ID FROM C_Currency WHERE o.CURRENCY = C_Currency.ISO_Code) ")
				.append(" WHERE Currency IS not NULL and C_PaymentTerm_ID IS NULL")
				.append(" AND I_IsImported<>'Y' AND IsSOTrx='N'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
...
//		End Customization LoH
		...
		//	Set IsSOTrx
		sql = new StringBuilder ("UPDATE I_InvoicePurchase o SET IsSOTrx='Y' ")
		...
		sql = new StringBuilder ("UPDATE I_InvoicePurchase o SET IsSOTrx='N' ")
...
//			log.warning ("Invalid Activity=" + no);
		...
		//	Set Country
		/**
		sql = new StringBuffer ("UPDATE I_InvoicePurchase o "
		...
		**/
		...
		sql = new StringBuilder ("SELECT * FROM I_InvoicePurchase ")
		...
		try
		{
			...
			while (rs.next ())
			{
				X_I_InvoicePurchase imp = new X_I_InvoicePurchase (getCtx(), rs, get_TrxName());
		...
		sql = new StringBuilder ("SELECT * FROM I_InvoicePurchase ")
			...
			.append(" ORDER BY C_BPartner_ID, C_BPartner_Location_ID, I_InvoicePurchase_ID");
		try
		{
			pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
			rs = pstmt.executeQuery ();
			...
			while (rs.next ())
			{
				X_I_InvoicePurchase imp = new X_I_InvoicePurchase (getCtx (), rs, null);

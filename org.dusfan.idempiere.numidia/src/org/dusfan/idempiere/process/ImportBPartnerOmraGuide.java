...
public class ImportBPartnerOmraGuide extends SvrProcess implements ImportProcess {
	...
	protected String doIt() throws Exception {
		...
		// Existe Partner in this group if the case set exist to i_is_imported = 'Y'
		sql = new StringBuilder("UPDATE I_ImportOmraBP i ")
		...
		// set package and vol
		sql = new StringBuilder("UPDATE I_ImportOmraBP i ")
				.append("SET M_product_ID =" + m_M_Product_ID + ", du_vol_id = "+ m_DU_Vol_ID)
				.append(" Where I_IsImported<>'Y'")
				.append(clientCheck).append(groupCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.CONFIG))
		log.config("package and vol =" + no);
		
		// check if code client is not guide
		sql = new StringBuilder("UPDATE I_ImportOmraBP i ")
				.append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Le code client n est pas guide, ' ")
				.append("WHERE CustomerCode <> '470299'").append(" AND I_IsImported<>'Y'").append(clientCheck)
				.append(groupCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.CONFIG))
			log.config("Le code client n est pas guide=" + no);
		...
		try
		{
			...
			while (rs.next()) {
				...
				order = createOrderWithLIne(bp, bpl, imp, visal);
				
				imp.setC_BPartner_ID(bp.getC_BPartner_ID());
				imp.setC_Order_ID(order.getC_Order_ID());
				imp.setM_Product_ID(m_M_Product_ID);
				imp.setDU_Vol_ID(m_DU_Vol_ID);
	...
	private MOrder createOrderWithLIne(MBPartner bp, MBPartnerLocation bpl, X_I_ImportOmraBP imp,
			MVisaGroupLine vline) {
		bp.load(get_TrxName());
		MOrder order = new MOrder(getCtx(), 0, get_TrxName());
		order.setClientOrg(imp.getAD_Client_ID(), imp.getAD_Org_ID());
		order.setC_DocTypeTarget_ID(1000047); // set comptoire for guide
		order.setDateOrdered(m_DateValue);
		order.setDateAcct(m_DateValue);
		int du_Season_Period_ID = DB.getSQLValue(get_TrxName(), "Select DU_Season_Period_ID from DU_Season_Period"+
				" where Trunc(current_date) between Trunc(startdate) and Trunc(enddate) ");
		...
		order.setBill_BPartner_ID(bp.getC_BPartner_ID());
		order.setBill_Location_ID(bpl.getC_BPartner_Location_ID());
		order.set_ValueNoCheck("C_BPartnerRelation_ID", bp.get_Value("C_BPartnerRelation_ID"));
		...
		order.set_ValueNoCheck("DU_Vol_ID", m_DU_Vol_ID); // set vol
		...
		l1.setM_Product_ID(m_M_Product_ID);
		l1.setQty(Env.ONE);
		l1.setC_Activity_ID(1000001);
		l1.saveEx();
		
		// create tax if exist
		if (imp.get_ValueAsInt("C_Charge_ID") > 0) {
			MOrderLine l2 = new MOrderLine(order);
			l2.setC_Charge_ID(imp.get_ValueAsInt("C_Charge_ID"));
			l2.setQty(Env.ONE);
